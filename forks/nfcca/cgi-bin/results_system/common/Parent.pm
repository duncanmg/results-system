...
If the -error argument is present, then that error object is used.
...
=cut
  ...
  #***************************************
    ...
    if ( $args{-config} ) {
      ...
      if ( ref( $self ) !~ m/ResultsConfiguration/ ) {
        my $c = $args{-error} ? ResultsConfiguration->new( -error => $args{-error} ) : ResultsConfiguration->new();
        if ( $c ) {
          $c->read_file;
