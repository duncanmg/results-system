...
=cut
  ...
  #***************************************
    ...
    $self->eAdd( "LeagueTable object created.", 1 );
...
=cut
  ...
  #***************************************
    ...
    if ( ! -d $dir ) {
      $self->eAdd( "Directory for csv files not found.", 5 );
      $err = 1;
    }
    ...
    if ( $err == 0 ) {
      ...
      if ( ! opendir $FP, $dir ) {
        $self->eAdd( "Unable to open directory $dir", 5 );
        $err = 1;
      }  
    }
    ...
    if ( $err == 0 ) {
      @files = readdir $FP;
      $self->eAdd( scalar( @files ) . " files retrieved from $dir.", 1 );
      close $FP;
    }
    ...
    if ( $err == 0 ) {
      ...
      $self->eAdd( scalar( @files ) . " of these files are week files for the division.", 1 );
    }
...
=cut
  ...
  #***************************************
    ...
    foreach my $f ( @$files_ref ) {
      ...
      my $wk = $f;
      $wk =~ s/^.*_(.*)\..*$/$1/;
      $self->eAdd( "Create WeekData object " . $self->get_division . " $wk", 1 );
      my $wd = WeekData->new( -config => $self->get_configuration, 
      ...
      if ( $wd ) {
        ...
        $self->eAdd( "Unable to create WeekData object.", 5 );
        $err = 1;        
      }
...
=cut
  ...
  #***************************************
    ...
    if ( ! $t_ref ) {
      $self->eAdd( "_sort_table(): No aggregated data to sort.", 2 );
      if ( $self->_get_sorted_table ) {
        $self->eAdd( "_sort_table(): A sorted list of teams already exists.", 2 );
        return 0;
      }
      else {
        $self->( "_sort_table(): No aggregated data and not list of teams.", 5 );
        return 1;        
      }
    }
    my @table = @$t_ref;
    ...
    if ( ! $sorter ) {
      $self->eAdd( "Unable to create sorter. " . $@, 5 );
      $err = 1;
    }
...
=cut
  ...
  #***************************************
    ...
    my $p = $c->get_path( "-cgi-dir" => "Y" ) . "/common/results_system.pl?page=tables_index&system=" . $q->param( "system" );
    ...
    $line = $q->h1( $c->get_descriptors( -title => "Y" ) . " " 
      . $c->get_descriptors( -season => "Y" ) ) . $line;
      
    my $s = $self->_get_sheet( "table_dir", "web" );
    # $s = $c->get_path( -cgi_custom => "y" ) . "/gen_styles.css";
    $line = $q->start_html( -style => $s, 
                            -title => $c->get_descriptors( -title => "Y" ) ) 
      . $line
      . $q->p( { -class => "timestamp" }, my $t = localtime ) # Force localtime into scalar context.
      . $q->end_html;
...
=cut
  ...
  #***************************************
    ...
    $self->eAdd( "Starting write_file():", 0 );
    if ( ! -d $dir ) {
      $self->eAdd( "Table directory $dir does not exist.", 5 );
      $err = 1;
    }
    if ( ! $line ) {
      $self->eAdd( "No data passed to write_file.", 5 );
      $err = 1;
    }
    else {
      $self->eAdd( $line, 1 );
    ...
    if ( $err == 0 ) {
      if ( ! open( $FP, ">", $f ) ) {
        $self->eAdd( "Unable to open file $f for writing.", 5 );
        $err = 1;
      }
      else {
        $self->eAdd( "File opened for writing. $f", 1 );
      }
    }
 
    if ( $err == 0 ) {
      my $ret=print $FP $line;
      $self->eAdd( "print statement returned a value of $ret", 1 );
      close $FP;
    }
    $self->eAdd( "Leaving write_file():err=" . $err, 0 );
    return $err;
  }
...
=cut
  ...
  #***************************************
    ...
    if ( $err == 0 && scalar( @files ) == 0 ) {
      $self->eAdd( "No week files found. Cannot produce table. Use teams in fixture list.", 1 );
      $is_week_data = 0;
      my $csv = $self->get_division;
      my $season = $self->get_configuration->get_season;
      my $dir = $self->get_configuration->get_path( -csv_files => "Y" );
      my $f = Fixtures->new( -full_filename => "$dir/$season/$csv", -error => $self->eGetError );
      if ( ! $f ) {
        $self->eAdd( "Unable to create Fixtures object.", 5 );
